	...
	def run(self):
		...
		while True:
			...
			for taddr in range(32):
				caddr = taddr + 1
				#print('Scanning addr {}'.format(caddr))
				...
				if frame == 'ERROR':
					...
					if (str(caddr) not in devlist) or (devlist[str(caddr)] != frame['DATA'][0]):
						devlist[str(caddr)] = {'HARDWARE':0,'ID':0}
						devlist[str(caddr)]['HARDWARE'] = frame['DATA'][0]
						devlist[str(caddr)]['ID'] = frame['DATA'][1]
						#devdataLock.acquire()
						devdata[str(caddr)] = []
